...
<form action="forms/firewall_form_submit.php" method="post" name="iform"
	id="iform"><input name="formname" type="hidden" value="system_routes">
<input name="id" type="hidden" value="<?=$id;?>"> <input name="oldif"
	type="hidden" value="<?=$pconfig['interface'];?>"> <input name="olddst"
	type="hidden" value="<?=$pconfig['network'];?>"> <input name="oldgw"
	type="hidden" value="<?=$pconfig['gateway'];?>"> <input
	name="oldrtable" type="hidden" value="<?=$pconfig['rtable'];?>">
...
<div><label for="network">Destination</label> <input name="network"
	type="text" class="formfld" id="network" size="20"
	value="<?=htmlspecialchars($pconfig['network']);?>"> / <select
	name="network_subnet" class="formfld" id="network_subnet">
	<?php for ($i = 32; $i >= 1; $i--): ?>
	<option value="<?=$i;?>"
	<?php if ($i == $pconfig['network_subnet']) echo "selected"; ?>><?=$i;?>
	</option>
	<?php endfor; ?>
</select>
...
<div><label for="rtable">Route Table</label> <select name="rtable"
	class="formfld">
