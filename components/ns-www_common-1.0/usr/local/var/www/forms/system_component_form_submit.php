			...
			if ($mode) {
				if ($mode == "upgrade") {
					if (is_uploaded_file($_FILES['ulfile']['tmp_name'])) {
						...
						if (!stristr($_FILES['ulfile']['name'], '.tgz') && !$_POST['sig_override'])
						$input_errors[] = "The uploaded image file is not for this platform ({$g['fullplatform']}).";
							...
							rename($_FILES['ulfile']['tmp_name'], "{$g['ftmp_path']}/".$_FILES['ulfile']['name']);
						}
					}
				...
				<form action="forms/system_component_form_submit.php" id="sigform" method="post">
                ...
                case "system_firmware_sig_override" :
                        if ($_POST['sig_override']) {
                                $savemsg = "The component is now being installed.";
                                echo "<center>$savemsg</center>";
                                echo '<script type="text/javascript">
                                         setTimeout(function(){ $("#upload_firmware").dialog("close"); }, 5000);
                                      </script>';
                                /* fire up the update script in the background */
                                exec_rc_script_async("/etc/rc.component_upgrade");
                                return 0;
                        } elseif ($_POST['sig_no']) {
                                unlink("{$g['ftmp_path']}/firmware.img");
