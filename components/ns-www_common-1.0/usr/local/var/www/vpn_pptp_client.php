...
<form action="forms/vpn_form_submit.php" method="post" name="iform"
	id="iform"><input name="formname" type="hidden" value="vpn_pptp_client">
<input name="id" type="hidden" value="<?=$id;?>">
<fieldset><legend><?=join(": ", $pgtitle);?></legend>
<div><label for="enable">Enable PPTP Client</label> <input name="enable"
	type="checkbox" id="enable" value="Yes"
	<?php if ($pconfig['enable']) echo "checked"; ?>>
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="connectonboot">Attempt Connection on Bootup</label> <input
	name="connectonboot" type="checkbox" id="connectonboot" value="Yes"
	<?php if ($pconfig['connectonboot']) echo "checked"; ?>>
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="server">Server Address</label> <input name="server"
	type="text" class="formfld" id="server" size="25"
	value="<?=htmlspecialchars($pconfig['server']);?>">
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="username">Username</label> <input name="username"
	type="text" class="formfld" id="username" size="10"
	value="<?=htmlspecialchars($pconfig['username']);?>">
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="password">Password</label> <input name="password"
	type="password" class="formfld" id="password" size="10"
	value="<?=htmlspecialchars($pconfig['password']);?>">
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="tunnelforce">PPTP Connection Force</label> <input
	name="tunnelforce" type="checkbox" id="forcetunnel" value="Yes"
	<?php if ($pconfig['forcetunnel']) echo "checked"; ?>>
<p class="note">Check this box to force all traffic to be routed to the
PPTP server instead of the WAN default gateway ip.</p>
</div>
<div><label for="tunnelforceoverride">PPTP Connection Force Override</label>
<input name="tunnelforceoverride" type="checkbox"
	id="tunnelforceoverride" value="Yes"
	<?php if ($pconfig['forcetunneloverride']) echo "checked"; ?>>
<p class="note">In the event of a PPTP tunnel failure, connections will
be sent to the WAN default gateway ip.</p>
</div>
<div id="routes"><label for="members">Remote Network(s)</label> <select
	name="MEMBERS" style="width: 160px; height: 100px" id="MEMBERS"
	multiple>
	<?php for ($i = 0; $i<sizeof($pconfig['routelist']); $i++): ?>
	<option value="<?=$pconfig['routelist']["route$i"];?>"><?=$pconfig['routelist']["route$i"];?>
	</option>
	<?php endfor; ?>
...
<div><label for="autonat">Generate NAT rules for PPTP</label> <input
	name="autonat" type="checkbox" id="autonat" value="Yes"
	<?php if ($pconfig['autonat']) echo "checked"; ?>>
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
<div><label for="lcplog">LCP debug log</label> <input name="lcplog"
	type="checkbox" id="lcplog" value="Yes"
	<?php if ($pconfig['lcplog']) echo "checked"; ?>>
<p class="note">You may enter a description here for your reference (not
parsed).</p>
</div>
