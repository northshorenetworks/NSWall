...
<form action="forms/system_form_submit.php" method="post" name="iform"
	...
	value="firewall_carp_config_sync_host">
...
<div><label for="ip">Config Sync Host IP</label> <input name="ip"
	type="text" class="formfld" id="ip" size="16"
	value="<?=htmlspecialchars($pconfig['ip']);?>"></div>
<!--           	<div>
...
<p class="note">The authentication password to use when talking to this
config sync host. This password must be configured on the host.</p>
</div>
<div><label for="port">Port</label> <input name="port" type="text"
	class="formfld" id="port" size="4"
	value="<?=htmlspecialchars($pconfig['port']);?>">
<p class="note">The port that the remote config sync host uses for
configuration.</p>
</div>
</fieldset>
