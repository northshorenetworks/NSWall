...
<form action="forms/interfaces_form_submit.php" method="post"
	...
	value="interface_pfsync">

<fieldset><legend><?=join(": ", $pgtitle);?></legend>
<div><label for="pfsyncenable">Enable PFSync</label> <input
	id="pfsyncenable" type="checkbox" name="pfsyncenable" value="Yes"
	<?php if ($pconfig['pfsyncenable']) echo "checked"; ?> />
<p class="note">Accept incoming PFSync packets</p>
</div>
<div><label for="pfsyncenable">Interface</label> <select
	name="interface" class="formfld">
	...
	<?php if ($ifacename == $pconfig['interface']) echo "selected"; ?>><?=htmlspecialchars($ifacename);?>
	</option>
	<?php endforeach; ?>
</select></div>
</fieldset>
