...
if ($_POST) {
	...
	switch($form) {
		...
		case "firewall_blockedsites":
			if (!is_array($config['filter']['blockedsites']))
			$config['filter']['blockedsites'] = array();
			$a_blockedsites = &$config['filter']['blockedsites'];

			unset($input_errors);
			$pconfig = $_POST;

			unset($a_blockedsites['memberlist']);
			$memberslist = explode(' ', $_POST['memberslist']);
			for($i=0;$i<sizeof($memberslist); $i++) {
				$member = 'member'."$i";
				$prop = preg_replace("/ /", "", $memberslist[$i]);
		...
		case "firewall_carp_config_sync_host":
...
if ($_GET) {
	...
	if ($type == 'abusive') {
		...
		$a_blockedsites = &$config['filter']['blockedsites'];
		if ($action == 'add') {
			$i = count($a_blockedsites['memberlist']);
			$member = 'member'."$i";
			echo $member;
			$a_blockedsites['memberlist'][$member] = $_GET['srcaddr'];
			write_config();
			filter_configure();
		}
	}
