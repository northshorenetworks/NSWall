...
#
# $Id: build-VMWareimage.sh,v 1.6 2008/12/03 02:09:54 jrecords Exp $
...
if [ ! $1 = "VMWARE" ]; then
   echo "stty com0 19200" >> ${MOUNTPOINT}/etc/boot.conf
   echo "set tty com0" >> ${MOUNTPOINT}/etc/boot.conf
fi
echo ""
...
${SUDO} cp ${BASE}/PLATFORM/$IMAGEFILE/config.xml ${MOUNTPOINT}/config.xml
...
#echo "Mounting DATA Partition to ${MOUNTPOINT}..."
#if ! ${SUDO} mount -o async /dev/${DEVICE}f ${MOUNTPOINT}; then
#  echo Mount failed..
#  exit
#fi
#echo ""
#echo "Copying default wb categories list..."
# Here is where you add your own packages and configuration to the flash...
#${SUDO} cp ${BASE}/binaries/nsfilters.tgz ${MOUNTPOINT}/nsfilters.tgz
...
echo ""
echo "Converting to VMWare Image format"
${SUDO} qemu-img convert -O vmdk $IMAGEFILE $IMAGEFILE.vmdk
