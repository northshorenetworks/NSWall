...
#define INC_STYLE	"/usr/www/bgplg.css"
#define INC_HEAD	"/usr/www/bgplg.head"
#define INC_FOOT	"/usr/www/bgplg.foot"

#define BGPDSOCK	"/var/run/bgpd.rsock"
...
main(void)
{
	...
	printf("Content-Type: %s\n"
	    ...
	    "<head>\n"
	    "<script>\n"
 	    "\t$(\"#bgplgsubmit\").click(function() {\n"		
	    "\t\tvar QueryString = $(\"#bgplgform\").serialize();\n" 
	    "\t\t$.get(\"/bgplg.cgi\", QueryString, function(output) {\n" 
	    "\t\t\t$(\"#livediag\").html(output);\n"    
 	    "\t\t});\n"
	    "\t\treturn false;\n"
	    "\t});\n"
	    "</script>\n"	
	...
	printf(
	    "<form id='bgplgform' action='%s'>\n"
	    "<div class=\"command\">\n"
	    "<select id='bgplgcmd' name='cmd'>\n",
	    self);
	...
	printf("</select>\n"
	    "<input id=bgplgreq' type='text' name='req'/>\n"
	    "<input id='bgplgsubmit' type='submit' value='submit'/>\n"
	    "</div>\n"
	    "</form>\n"
	    "<pre>\n");
	fflush(stdout);
