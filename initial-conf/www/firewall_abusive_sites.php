#!/bin/php
<?php
$pgtitle = array("Firewall", "Dynamic", "Blocked Sites");
...
$(document).ready(function() {
     ...
     $("#submitbutton").click(function () {
	  ...
	  $.post("forms/firewall_form_submit.php", QueryString, function(output) {
			   ...
			   setTimeout(function(){ $('#content').load('firewall_blockedsiteses_tabs.php'); }, 1250);
	  });
	  return false;
     });
...
<div id="wrapper">
	...
	<fieldset>
        ...
        <?php for ($i = 0; $i<sizeof($pconfig['memberlist']); $i++): ?>
				...
				<div>
                  <label for="dynamictimeout">Dynamic Blocked Sites expiration</label>
                  <input name="dynamictimeout" type="text" class="formfld" id="dynamictimeout" size="5" value="<?=htmlspecialchars($pconfig['dynamictimeout']);?>">
                  <p class="note">The amount of time in minutes that dynamic blocked sites expire from the list</p>
                </div>
                 <div>
                  <label for="members">Blocked Sites Exceptions</label>
                  <select name="NONMEMBERS" style="width: 160px; height: 100px" id="NONMEMBERS" multiple>
                  <?php for ($i = 0; $i<sizeof($pconfig['nonmemberlist']); $i++): ?>
                <option value="<?=$pconfig['nonmemberlist']["nonmember$i"];?>">
                <?=$pconfig['nonmemberlist']["nonmember$i"];?>
                </option>
                <?php endfor; ?>
        </select>
                <input type=button id='exceptionremove' value='Remove Selected'><br><br>
                  <label for="members">Type</label>
                    <select name="exceptionsrctype" class="formfld" id="exceptionsrctype">
                      <option value="exceptionsrchostdiv" selected>Host</option>
                      <option value="exceptionsrcnetdiv" >Network</option>
                      <option value="exceptionsrcblockedsitesdiv" >Alias</option>
        			  <option value="exceptionsrcuserdiv" >User</option>
		            </select>
                </div>
                <div id='exceptionsrchostdiv' style="display:block;">
                 <label for="srchost">Address</label>
                  <input name="srchost" type="text" class="formfld" id="exceptionsrchost" size="16" value="<?=htmlspecialchars($pconfig['address']);?>">
                <input type=button id='exceptionhostaddbutton' value='Add'>
                </div>
                <div id='exceptionsrcnetdiv' style="display:none;">
                 <label for="srcnet">Address</label>
                  <input name="srcnet" type="text" class="formfld" id="exceptionsrcnet" size="16" value="<?=htmlspecialchars($pconfig['address']);?>">
                   <strong>/</strong>
                    <select name="srcmask" class="formfld" id="exceptionsrcmask">
                      <?php for ($i = 30; $i >= 1; $i--): ?>
                      <option value="<?=$i;?>" <?php if ($i == $pconfig['address_subnet']) echo "selected"; ?>>
                      <?=$i;?>
                      </option>
                ...
                <input type=button id='exceptionnetaddbutton' value='Add'>
                </div>
                <div id='exceptionsrcblockedsitesdiv' style="display:none;">
                 <label for="srcblockedsites">Table</label>
                    <select name="srcblockedsites" class="formfld" id="exceptionsrcblockedsites">
                      <?php foreach($config['blockedsiteses']['blockedsites'] as $i): ?>
                      <option value="<?='$' . $i['name'];?>" <?php if ($i == $pconfig['address_subnet']) echo "selected"; ?>>
                        <?=$i['name'];?>
                      </option>
                ...
                <input type=button value='Add'>
               	...
               	<div id='exceptionsrcuser' style="display:none;">
                <strong>User</strong>
                    <select name="srcuser" class="formfld" id="exceptionsrcuser">
                      <?php foreach($config['system']['accounts']['user'] as $i): ?>
                      <option value="<?=$i['name'];?>">
                        <?=$i['name'];?>
                      </option>
