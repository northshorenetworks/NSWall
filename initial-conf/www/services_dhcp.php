...
<form action="forms/services_form_submit.php" method="post"
	name="<?=$formname?>" id="<?=$formname?>"><input name="formname"
	type="hidden" value="services_dhcpd">

<fieldset><legend><?=join(": ", $pgtitle);?></legend>
<div><label for="enable">Enable DHCP Server</label> <input id="enable"
	type="checkbox" name="enable" value="Yes"
	<?php if ($pconfig['enable']) echo "checked"; ?> />
<p class="note">Enable DHCP server on <?=htmlspecialchars($iflist[$if]);?>
interface.</p>
<input name="if" type="hidden"
	value="<?=htmlspecialchars($iflist[$if]);?>"></div>
<div><label for="subnet">Subnet</label> <?=gen_subnet($ifcfg['ipaddr'], $ifcfg['subnet']);?>
<p class="note"></p>
<label for="subnet">Subnet mask</label> <?=gen_subnet_mask($ifcfg['subnet']);?>
<p class="note"></p>
<label for="subnet">Availible Range</label> <?=long2ip(ip2long($ifcfg['ipaddr']) & gen_subnet_mask_long($ifcfg['subnet']));?>
- <?=long2ip(ip2long($ifcfg['ipaddr']) | (~gen_subnet_mask_long($ifcfg['subnet']))); ?>
<p class="note"></p>
</div>
<div><label for="range_from">Range</label> <input name="range_from"
	type="text" class="formfld" id="range_from" size="20"
	value="<?=htmlspecialchars($pconfig['range_from']);?>"> &nbsp;to&nbsp;
<input name="range_to" type="text" class="formfld" id="range_to"
	size="20" value="<?=htmlspecialchars($pconfig['range_to']);?>">
...
<div><label for="gateway">Gateway</label> <input name="gateway"
	...
	value="<?=htmlspecialchars($pconfig['gateway']);?>">
...
<div><label for="dns1">DNS Servers</label> <input name="dns1"
	...
	value="<?=htmlspecialchars($pconfig['dns1']);?>"> &nbsp; <input
	name="dns2" type="text" class="formfld" id="dns2" size="20"
...
<div><label for="wins1">Wins Servers</label> <input name="wins1"
	...
	value="<?=htmlspecialchars($pconfig['wins1']);?>"> &nbsp; <input
	name="wins2" type="text" class="formfld" id="wins2" size="20"
...
<div class="buttonrow"><input type="submit" id="<?=$ifsubmit;?>"
