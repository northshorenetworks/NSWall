...
<form id="iform" name="iform"><span id="loading">
<center>Loading NSWall Setup Utility<br>
<br>
<img src="images/ajax-loader.gif" height="25" width="25" name="spinner">
</center>
</span> <span id="welcome" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: Welcome To NSWall</legend>
<div>
<center><img alt="NSS Logo" class="png" width="50" height="50"
	src="/images/logo.jpg" /></center>
<br>
<center>Welcome to NorthShore Software's NSWall</center>
<br>
<center>The following wizard will guide you through the initial setup of
the appliance</center>
<br>
</div>
</fieldset>
<div class="buttonrow"><input type="submit" id="welcome_next_btn"
	value="Next" class="button" /></div>
</span> <span id="general" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: General Settings</legend>
...
<div><label for="domain">Domain</label> <input id="domain" type="text"
	name="domain" value="<?=htmlspecialchars($pconfig['domain']);?>" />
<p class="note">e.g. <em>mycorp.com</em></p>
</div>
<div><label for="dns1 dns2">DNS Servers</label> <input id="dns1"
	type="text" name="dns1"
	value="<?=htmlspecialchars($pconfig['dns1']);?>" size="20" /> <input
	id="dns2" type="text" name="dns2"
	value="<?=htmlspecialchars($pconfig['dns2']);?>" size="20" /></div>
...
<div class="buttonrow"><input type="submit" id="general_back_btn"
	value="Back" class="button" /> <input type="submit"
	id="general_next_btn" value="Next" class="button" /></div>
</span> <span id="wan" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: WAN Configuration</legend>
<div><label for="wantype">Connection Type</label> <select name="wantype"
	class="formfld" id="wantype">
	<option value="DHCP" selected>DHCP</option>
	<option value="Static">Static IP</option>
</select></div>
<div id='staticdiv'>
<div><label for="wanipaddr">IP address</label> <input name="wanipaddr"
	type="text" class="formfld" id="wanipaddr" size="20"
	value="<?=htmlspecialchars($pconfig['wanipaddr']);?>"> / <select
	name="wansubnet" class="formfld" id="subnet">
	<?php for ($i = 31; $i > 0; $i--): ?>
	<option value="<?=$i;?>"
	<?php if ($i == $pconfig['wansubnet']) echo "selected"; ?>><?=$i;?></option>
	<?php endfor; ?>
</select></div>
</div>
</fieldset>
<div class="buttonrow"><input type="submit" id="wan_back_btn"
	value="Back" class="button" /> <input type="submit" id="wan_next_btn"
	value="Next" class="button" /></div>
</span> <span id="lan" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: LAN Configuration</legend>
<div><label for="lanipaddr">IP address</label> <input name="lanipaddr"
	type="text" class="formfld" id="lanipaddr" size="20"
	value="<?=htmlspecialchars($pconfig['lanipaddr']);?>"> / <select
	name="lansubnet" class="formfld" id="lansubnet">
	<?php for ($i = 31; $i > 0; $i--): ?>
	<option value="<?=$i;?>"
	<?php if ($i == $pconfig['lansubnet']) echo "selected"; ?>><?=$i;?></option>
	<?php endfor; ?>
</select></div>
</fieldset>
<div class="buttonrow"><input type="submit" id="lan_back_btn"
	value="Back" class="button" /> <input type="submit" id="lan_next_btn"
	value="Next" class="button" /></div>
</span> <span id="wifi" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: WIFI Configuration</legend>
<div><label for="wifienable">Enable Wireless IF</label> <input
	id="wifienable" type="checkbox" name="wifienable" value="Yes"
	<?php if ($pconfig['enable']) echo "checked"; ?> /></div>

<div><label for="wifiifmode">Wireless Mode</label> <select
	name="wifiifmode" class="formfld" id="wifiifmode">
	<?php $modes = array('lanbridge' => 'Bridge to LAN', 'nobridge' => 'Independant Network');
	foreach ($modes as $mode => $modename): ?>
	<option value="<?=$mode;?>"
	<?php if ($mode == $pconfig['ifmode']) echo "selected"; ?>><?=htmlspecialchars($modename);?>
	</option>
	<?php endforeach; ?>
</select></div>
<div id="ipdiv"><label for="wifiipaddr">IP address</label> <input
	name="wifiipaddr" type="text" class="formfld" id="wifiipaddr" size="20"
	value="<?=htmlspecialchars($pconfig['wifiipaddr']);?>"> / <select
	name="wifisubnet" class="formfld" id="wifisubnet">
	<?php for ($i = 31; $i > 0; $i--): ?>
	<option value="<?=$i;?>"
	<?php if ($i == $pconfig['wifisubnet']) echo "selected"; ?>><?=$i;?></option>
	<?php endfor; ?>
</select></div>
...
<div id="wpadiv"><label for="wpacipher">Preshared Key</label> <input
	name="wpapsk" type="password" class="formfld" id="wpapsk" size="30"
	value="<?=htmlspecialchars($pconfig['wpapsk']);?>"></div>
</fieldset>
<div class="buttonrow"><input type="submit" id="wifi_back_btn"
	value="Back" class="button" /> <input type="submit" id="wifi_next_btn"
	value="Next" class="button" /></div>
</span> <span id="summary" style="display: none";>
<fieldset><legend><?=join(": ", $pgtitle);?>: Welcome To NSWall</legend>
<div>
<center><img alt="NSS Logo" class="png" width="50" height="50"
	src="/images/logo.jpg" /></center>
<br>
<center>Your Device is now ready to be configured</center>
<br>
<center>The following settings will be applied to your appliance after
clicking "Submit"</center>
<br>
</div>
<center><input type="submit" id="wizard_submit_btn" value="Submit"
	class="button" /></center>
</fieldset>
<div class="buttonrow"><input type="submit" id="summary_back_btn"
